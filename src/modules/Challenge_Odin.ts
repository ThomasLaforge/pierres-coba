import { DiceFaceOdin as DiceFace, Side } from "./definitions";
...
export class Challenge {
    ...
    getTotal(part: DiceFace[], otherPart: DiceFace[]){
        let scores = part.map(face => {
            if(face === DiceFace.Green_Soldat){
                return 1
            }
            else if(face === DiceFace.Grey_Capitaine){
                return 2
            }
            else if(face === DiceFace.Purple_Hero){
                return 3
            }
            else if(face === DiceFace.Red_Traitre){
                return 1
            }
            else if(face === DiceFace.Yellow_Maudit){
                return -1
            }
            else if(face === DiceFace.Blue_Mage){
                return part.filter(df => df < 6 && df !== DiceFace.Blue_Mage).length
            }
            else {
                return null
            }
        ...
        const nbRedDices = part.filter(df => df === DiceFace.Red_Traitre).length
        for (let i = 0; i < nbRedDices; i++) {
            const indexPurpleDice = part.indexOf(DiceFace.Purple_Hero)
            if(indexPurpleDice !== -1){
                scores[indexPurpleDice] = 0
            }
        }
        console.log('scores after red', scores)
